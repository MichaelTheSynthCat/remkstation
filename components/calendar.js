...
function CalendarTable({view_date, dataRef}){
    ...
    const generateCells = (main_date, dataRef) => {
        ...
        mondays.forEach( (monday) => {
            cells.push(<CalendarDayEntry key={monday.toDateString()} date={monday} dataRef={dataRef}/>);
            ...
            for(var i=1; i<7;i++){
                ...
                cells.push(<CalendarDayEntry key={week_days[i].toDateString()} date={week_days[i]} dataRef={dataRef}/>);
            }
        })
        return cells;
    }
    ...
    return (
        <div className="grid grid-cols-7 gap-2 place-items-center">{generateCells(view_date, dataRef)}</div>
    )
}

function CalendarAddEvent({handleOpenDialog}){
    return (
        <Button variant="outlined" onClick={() => handleOpenDialog()}>+</Button>
    );
}
...
function CalendarHeader({view_date, handleViewChange, handleOpenDialog}){
    var actual_month = (main_date) => {
        return (
            <span>{main_date.getFullYear().toString() + "/" + main_date.toLocaleString('default', { month: 'long' })}</span>
        )
    } 
    ...
    return (
        <div>
            ...
            <Button variant="outlined" onClick={() => handleViewChange(-1)}>{"<"}</Button>
            <Button variant="outlined" onClick={() => handleViewChange(1)}>{">"}</Button>
...
function EventEditDialog({openBool, handleCloseDialog, handleSaveChanges}){
    ...
    return (
        <Dialog open={openBool}>
            <div>New event</div>
            <TextField id="name" variant="outlined" label="Name" value={name} onChange={(e) => {setName(e.target.value)}}/>
            <TextField id="description" variant="outlined" label="Description" value={description} onChange={(e) => {setDescription(e.target.value)}}/>
            <div>
                <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale="en-gb">
                    <DatePicker label="Date" value={date} onChange={(newDate) => setDate(newDate)}/>
                    <TimePicker label="Time" value={date} onChange={(newDate) => setDate(newDate)} />
                </LocalizationProvider>
            </div>
            <div>
                <Button variant="outlined" onClick={() => handleCloseDialog()}>Cancel</Button>
                <Button variant="outlined" onClick={() => {handleSaveChanges(name, description, date); setName("")}}>Add</Button>
            </div>
        </Dialog>
    )
}
...
export default function Calendar(){
    ...
    const [editedEvent, setEditedEvent] = useState({id:"", name:"", date:null});
    ...
    function changeMonth(increment){ // increment is +1 or -1
    ...
    const handleSaveChanges = (name, description, date) => {
        ...
        var newEvent = {id: id, name:name_str, description:description_str, date:dateJS};
        addEvent(newEvent);
        setOpenDialog(false);
    }
    ...
    return (
        <div className="w-full place-self-start">
            <CalendarHeader view_date={viewDay} handleViewChange={changeMonth} handleOpenDialog={handleOpenAddDialog}/>
            <CalendarTable view_date={viewDay} dataRef={calendarData}/>
            <EventEditDialog openBool={openDialog} handleCloseDialog={handleCloseDialog} handleSaveChanges={handleSaveChanges}/>
        </div>
    )
}